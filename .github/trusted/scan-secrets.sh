...
# Candidate completion evidence is bound to the frozen candidate graph, not
# ambient stashes or unrelated branches. Annotated tag metadata remains in
# scope so a tag message cannot hide secret material.
...
if [ -z "$object_ids" ]; then
  echo "PASS: selected Git object set is empty."
  exit 0
fi
...
if [ "${#tree_ids[@]}" -gt 0 ]; then
  tree_entries_file="$(mktemp)" || { echo "BLOCKED: cannot create temporary tree storage."; exit 2; }
  ...
  for tree_id in "${tree_ids[@]}"; do
    if ! git ls-tree -z --name-only "$tree_id" >"$tree_entries_file" 2>/dev/null; then
      echo "BLOCKED: cannot inspect selected Git tree paths."
      exit 2
    fi
...
if [ "$scan_error" -ne 0 ]; then
  echo "BLOCKED: cannot safely scan selected Git object content."
  exit 2
fi
...
echo "PASS: selected Git objects contain no detected secrets."
