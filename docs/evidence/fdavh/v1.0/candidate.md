...
Normative Specification v1.0 — PROPOSED FOR LOCK
Date: 2026-08-28
...
VERDICT: PASS — NORMATIVE SPECIFICATION / IMPLEMENTATION CONTRACT. This artifact is sufficiently complete to begin implementation of the deterministic harness. It does not certify that any implementation exists or is verified. Production PASS requires implementation plus the acceptance and TEVV evidence in this specification.
...
|R7 |Section-Level Routing                                 |Large governing documents MUST be routed at the smallest authoritative section granularity sufficient for the task.                                                                    |
|R8 |No Lossy Compression of Normative Rules               |Context optimization MUST remove irrelevant material before summarizing; mandatory normative obligations MUST be exact or losslessly compiled.                                         |
...
|R14|Context Budget Is Hard but Subordinate to Completeness|A context budget MUST NOT silently truncate required governance; exact section routing, qualified lossless compilation, or BLOCKED are the only conforming outcomes.                   |
...
|MR3 |Equivalent Authoritative Context          |Candidate models for the same task state MUST receive the same authoritative route contract; model-specific formatting MUST preserve semantic equivalence. |
...
|---|---------------------|---------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------|
|S1 |ExecutionEnvelope    |run_id, task_id, repository_id, base_state, delegator_id, workload_identity, goal, scope, purpose, authority_set, capability_set, risk_class, budgets, expiry, governance_kernel_digest; immutable after activation except through external authority transition.                                        |
...
|S3 |ModelRouteDecision   |decision_id, run_id, route_id, candidate_pool_digest, selected_model_id, provider_id, policy_id, routing_memory_refs, verifier_grounded_evidence_refs, cost/latency constraints, decision_digest; MUST NOT contain authority mutation.                                                                   |
|S4 |InferenceReceipt     |inference_id, run_id, step_id, sequence, nonce, operation_type, workload_identity, route_digest, context_digest, provider_id, model_id, config_digest, request_digest, dispatch_time, provider_receipt, raw_response_digest, token/cost/time usage, admission_status.                                    |
|S5 |ActionAuthorization  |action_id, run_id, step_id, proposal_digest, tool_id, operation, canonical_arguments_digest, target_resources, purpose, capability_id, reversibility_class, chain_reversibility_class, preconditions, required_approval, approval_evidence_ref, nonce, budget_snapshot, decision, policy_digest.         |
...
|S7 |FailureLearningRecord|failure_id, run_id, step_id, failed_invariant_ids, failure_class, checkpoint_id, expected_state_digest, actual_state_digest, evidence_refs, prior_occurrence_refs, materially_new_evidence, causal_status, allowed_recovery_ids, remaining_budget, recorded_at.                                          |
|S8 |RecoveryCheckpoint   |checkpoint_id, run_id, plan_step_id, goal_digest, authority_digest, state_digest, required_postcondition, allowed_recovery_ids, return_transition, retry_budget_snapshot; immutable during candidate repair.                                                                                             |
...
|EFFECT_VERIFIED      |MODEL_SELECTED                              |more work remains                                                                  |next authorized checkpoint; governance unchanged                              |
...
|REPAIR_VALIDATION    |RECOVERABLE_FAIL                            |repair validation FAIL with materially new evidence and budget                     |new S7; another predefined recovery exists                                    |
...
|BLOCKED              |BLOCKED                                     |preserve/report/await only                                                         |NO autonomous exploration, retry, redesign, substitution, or search           |
|BLOCKED              |GOVERNANCE_ROUTED or DELEGATED              |authenticated external unblock transition                                          |new evidence/authority satisfies predefined transition; full revalidation     |
...
1. Create S7 before any retry, reroute, repair, or stop decision.
...
|---------|------------------|------------------------------------------------------|---------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------|-------------|-------------|-----------|-----------------|
|I1       |Runtime           |External Authority                                    |No consequential effect executes unless the acting principal and delegated authority are independently established outside model-controlled state.                                     |G0/G7        |S1,S5,P1     |E4,E8      |AC-01,AC-25      |
|I2       |Runtime           |Trust Integrity                                       |Model-controlled or untrusted information MUST NOT promote its own trust level or manufacture authority.                                                                               |G1/G4/G7     |S2,S5,S6,P5  |E4,E7,E8   |AC-06,AC-44      |
|I3       |Runtime           |Complete Mediation                                    |Every consequential operation MUST cross the deterministic runtime gateway; no alternate effect path is authoritative.                                                                 |G7/G8        |S5,S6,P1     |E3,E7,E8   |AC-14,AC-21,AC-29|
...
|I5       |Runtime           |Transition Safety                                     |Every admitted state transition MUST preserve all hard invariants and satisfy the active transition guard.                                                                             |G6/G7/G8/G9  |S5,S6,S9,P2  |E6,E8      |AC-20,AC-30      |
|I6       |Runtime           |Observability                                         |Every fact required for verification MUST be externally observable from a trusted vantage; missing evidence MUST NOT become PASS.                                                      |G9/G12       |S6,S9        |E2-E8      |AC-28,AC-40      |
...
|I9       |Runtime           |Gate-State Integrity                                  |The model MUST NOT create, rename, reopen, resolve, or substitute governance gates or their states.                                                                                    |G1/G11       |S2,S9,P2     |E4,E8      |AC-01            |
...
|I16      |Runtime           |Fixed Recovery Horizon                                |Recovery limits for retries, inference calls, tool calls, elapsed time, mutations, changed surface, and cost MUST be supervisor-owned and non-resettable by the model.                 |G0/G3/G10/G11|S1,S4,S7,S8  |E8         |AC-37            |
|I17      |Runtime           |Reversible Autonomous Recovery                        |Autonomous recovery MUST be isolated and reversible; irreversible or externally durable effects require stronger pre-execution authority.                                              |G7/G10       |S5,S8,P1     |E4,E8      |AC-24,AC-25,AC-33|
...
|I23      |Runtime           |BLOCKED MUST NOT Trigger Exploration                  |In BLOCKED, autonomous exploration, search, redesign, substitution, tool creation, alternate-source discovery, retries, and scope expansion MUST be unavailable.                       |G11          |S9,P2        |E8         |AC-02,AC-38      |
...
|IV9      |Inference         |Inference Budget Enforcement                          |Token, call, time, cost, retry, and lease limits MUST be supervisor-owned and checked before invocation.                                                                               |G3           |S1,S4        |E8         |AC-37            |
...
|R7       |Governance routing|Section-Level Routing                                 |Large governing documents MUST be routed at the smallest authoritative section granularity sufficient for the task.                                                                    |G1           |S2,P3        |E8         |AC-42            |
|R8       |Governance routing|No Lossy Compression of Normative Rules               |Context optimization MUST remove irrelevant material before summarizing; mandatory normative obligations MUST be exact or losslessly compiled.                                         |G1/G4        |S2           |E8         |AC-42,AC-43      |
...
|R12      |Governance routing|Memory Has No Normative Authority                     |Historical trajectories, embeddings, prior agent notes, failure memory, or routing memory MUST NOT alter scope, gate state, acceptance criteria, required evidence, or stop conditions.|G1/G2        |S2,S3        |E8         |AC-44            |
...
|R14      |Governance routing|Context Budget Is Hard but Subordinate to Completeness|A context budget MUST NOT silently truncate required governance; exact section routing, qualified lossless compilation, or BLOCKED are the only conforming outcomes.                   |G1           |S2           |E8         |AC-42            |
...
|MR3      |Model routing     |Equivalent Authoritative Context                      |Candidate models for the same task state MUST receive the same authoritative route contract; model-specific formatting MUST preserve semantic equivalence.                             |G2/G4        |S2,S3,S4     |E7,E8      |AC-09            |
...
|MR7      |Model routing     |Routing Memory Is Advisory                            |Routing Memory may influence model choice but MUST NOT alter governance or authority.                                                                                                  |G2           |S3           |E8         |AC-44            |
...
|MR13     |Model routing     |Repeated Rerouting Needs New Evidence                 |Repeated model routing without materially new verifier evidence MUST transition to BLOCKED when the configured reroute budget is exhausted.                                            |G2/G10/G11   |S7,S9        |E8         |AC-32,AC-37      |
...
• SPEC-CONFORMANT: design implements all 74 invariants, schemas, static manifests, state machine, and required gates.
• TEST-CONFORMANT: SPEC-CONFORMANT plus all AC-01–AC-48 and TEVV requirements PASS.
• RUNTIME-CONFORMANT: TEST-CONFORMANT plus real inference/tool/provider/sandbox paths demonstrate complete mediation and fail-closed behavior.
• REPOSITORY-MERGE-ELIGIBLE: RUNTIME-CONFORMANT plus repository-specific exact-head CI/review requirements PASS. This state still does not grant merge authority.
