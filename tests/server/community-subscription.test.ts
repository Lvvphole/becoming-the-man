...
function dependencies(
  providerStatus: "not_synced" | "reachable" = "not_synced",
): { calls: string[]; value: CommunitySubscriptionDependencies } {
  const calls: string[] = [];
...
describe("community subscription", () => {
  it("rejects missing marketing consent before persistence or provider calls", async () => {
    ...
    await expect(
      subscribeToCommunity({ ...input, marketingConsent: false }, value),
    ).resolves.toEqual({
      status: "error",
      requestId: input.requestId,
      error: {
  ...
  it("persists consent and subscriber state before synchronizing Resend", async () => {
    ...
    expect(value.audienceProvider.syncEligibleContact).toHaveBeenCalledWith({
      ...
      idempotencyKey: `community-contact:${input.requestId}`,
    });
  });

  it("returns pending and preserves durable state when Resend synchronization fails", async () => {
    const { calls, value } = dependencies();
  ...
  it("does not repeat the provider side effect when the request is already reachable", async () => {
